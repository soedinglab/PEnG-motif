...
Peng::Peng(const int pattern_length, Strand s, const int k, SequenceSet* sequence_set,
           BackgroundModel* bg) {
  ...
  if (pattern_length > std::log(SIZE_MAX) / std::log(IUPAC_ALPHABET_SIZE) - 1
    ...
    std::cerr
        << "max pattern length: "
        << std::max(std::log(SIZE_MAX) / std::log(IUPAC_ALPHABET_SIZE) - 1,
                    std::log(SIZE_MAX) / std::log(Alphabet::getSize()) - 1)
        << std::endl;
    exit(1);
  }
  ...
  for (size_t pattern = 0; pattern < this->number_patterns; pattern++) {
    if (this->pattern_counter[pattern] == 0) {
      ...
      if (this->pattern_counter[pattern] > mu && this->pattern_counter[pattern] > 5) {
        this->pattern_logp[pattern] = this->pattern_counter[pattern]
            * log(mu / this->pattern_counter[pattern]) + this->pattern_counter[pattern] - mu
            - 0.5 * log(6.283 * this->pattern_counter[pattern] * frac * frac);
      } else {
...
#pragma omp parallel for schedule(static)
  for (size_t pattern = 0; pattern < this->number_patterns; pattern++) {
    this->pattern_zscore[pattern] = (this->pattern_counter[pattern]
        - ltot * pattern_bg_probabilities[pattern])
        / sqrt(ltot * pattern_bg_probabilities[pattern]);
  }
}

void Peng::calculate_bg_probabilities(const int alphabet_size, const int k, GapMask* mask,
                                      BackgroundModel* model, size_t* factors,
                                      float* full_mask_patterns) {
...
//      std::cout << "\t\t" << BasePattern::toString(get_bg_id(pattern, k_prime+1, k_prime)) << "\t" << model->getV()[k_prime][get_bg_id(pattern, k_prime+1, k_prime)] << std::endl;
    ...
    calculate_bg_probability(background_model, alphabet_size, mask->get_mask_length(), k,
                             mask->get_mask_length() - k - 1, pattern, cur_prob, factors,
                             full_mask_patterns);
...
void Peng::calculate_bg_probability(float* background_model, const int alphabet_size,
                                    const int pattern_length, const int k,
                                    int missing_pattern_length, size_t pattern, float cur_prob,
                                    size_t* factors, float* final_probabilities) {
...
//    std::cout << "\t\t" << BasePattern::toString(kmer_id) << "\t" << background_model[kmer_id] << std::endl;
    if (missing_pattern_length == 0) {
      ...
      calculate_bg_probability(background_model, alphabet_size, pattern_length, k,
                               missing_pattern_length, extended_pattern, extended_pattern_prob,
                               factors, final_probabilities);
    }
  }
}
...
void Peng::count_patterns(const int pattern_length, const int alphabet_size,
                          const size_t number_patterns, GapMask* mask, SequenceSet* sequence_set,
...
//      std::cerr << BasePattern::toString(id) << std::endl;
      ...
      if ((pattern_plus_positions.find(id) == pattern_plus_positions.end()
          || i - pattern_plus_positions[id] >= pattern_length)
          && (pattern_minus_positions.find(id) == pattern_minus_positions.end()
              || i - pattern_minus_positions[id] >= pattern_length)
          && (pattern_plus_positions.find(rev_id) == pattern_plus_positions.end()
              || i - pattern_plus_positions[rev_id] >= pattern_length)
          && (pattern_minus_positions.find(rev_id) == pattern_minus_positions.end()
...
void Peng::filter_base_patterns(const int pattern_length, const int alphabet_size,
                                ...
                                const size_t count_threshold, float* pattern_zscore,
...
void Peng::em_optimize_pwms(std::vector<IUPACPattern*>& best_iupac_patterns,
                            const float saturation_factor, const float min_em_threshold,
...
#pragma omp parallel for
  for (int i = 0; i < best_iupac_patterns.size(); i++) {
    ...
    while (true) {
      ...
      for (size_t pattern = 0; pattern < number_patterns; pattern++) {
        for (int p = 0; p < pattern_length; p++) {
          ...
          new_pwm[p][a] += pattern_counter[pattern] * saturation_factor
              / (1 + saturation_factor / prob_odds[pattern]);  //saturation_factor / (1 + saturation_factor / prob_odds[pattern]);// prob_odds[pattern]
        }
      }
    ...
    std::cout << "em: " << IUPACPattern::toString(ori_pattern, pattern_length) << " -> "
              << best_iupac_patterns[i]->get_pattern_string() << std::endl;
...
void Peng::init_prob_odds(const size_t pattern_length, size_t curr_pattern, float curr_prob,
                          int curr_length, float** pwm, float* pattern_bg_probabilities,
                          float* prob_odds) {
  if (curr_length < pattern_length) {
    ...
    for (int a = 0; a < 4; a++) {
      ...
      init_prob_odds(pattern_length, next_pattern, next_prob, next_pattern_length, pwm,
                     pattern_bg_probabilities, prob_odds);
    }
  } else {
...
void Peng::merge_iupac_patterns(const size_t pattern_length, const float merge_bit_factor_threshold,
  ...
  while (found_better) {
    ...
    if (best_score > pattern_length * merge_bit_factor_threshold) {
      ...
      if (best_iupac_patterns[best_i]->get_pattern_length()
          < best_iupac_patterns[best_j]->get_pattern_length()) {
        merged_pattern = new IUPACPattern(best_iupac_patterns[best_j], best_iupac_patterns[best_i],
                                          best_comp, bg_model->getV()[0], best_shift);
      } else {
...
void Peng::process(const float zscore_threshold, const size_t count_threshold,
                   const int pseudo_counts, const bool use_em, const float em_saturation_factor,
                   const float min_em_threshold, const int em_max_iterations,
                   const bool use_merging, const float bit_factor_merge_threshold,
                   const bool adv_pwm, std::vector<IUPACPattern*>& best_iupac_patterns) {
  ...
  for (int i = 0; i < masks.size(); i++) {
    ...
    count_patterns(pattern_length, Alphabet::getSize(), number_patterns, gap_mask, sequence_set,
                   pattern_counter);
    if (this->strand == BOTH_STRANDS) {
      count_patterns_minus_strand(pattern_length, Alphabet::getSize(), number_patterns,
                                  pattern_counter);
    }
    ...
    if (gap_mask->get_mask_length() != last_gap_mask_length) {
      ...
      for(size_t i = 0; i < gap_mask->get_mask_length() + 1; i++) {
        factors[i] = pow(Alphabet::getSize(), i);
      }
      ...
      calculate_bg_probabilities(Alphabet::getSize(), k, gap_mask, bg_model, factors,
                                 full_mask_patterns);
    }
    ...
    filter_base_patterns(pattern_length, Alphabet::getSize(), number_patterns, zscore_threshold,
                         count_threshold, pattern_zscore, filtered_base_patterns);
    ...
    for (auto pattern : iupac_patterns) {
      std::cerr << "iupac pattern: "
                << IUPACPattern::toString(pattern->get_pattern(), pattern_length) << std::endl;
    }

    filter_iupac_patterns(iupac_patterns);
    for (auto pattern : iupac_patterns) {
      std::cerr << "selected iupac pattern: "
                << IUPACPattern::toString(pattern->get_pattern(), pattern_length) << std::endl;
    }
    ...
    for (auto pattern : iupac_patterns) {
      std::cerr << "adv pwm: " << IUPACPattern::toString(pattern->get_pattern(), pattern_length)
                << " -> " << pattern->get_pattern_string() << std::endl;
    }
  ...
  if (use_merging) {
    if (pattern_length >= MIN_MERGE_OVERLAP) {
      ...
      std::cerr << "Warning: Specified pattern length (" << pattern_length
                << ") is too low for merging!" << std::endl;
    }
  }
...
void Peng::optimize_iupac_patterns(std::vector<size_t>& selected_base_patterns,
  ...
  for (auto pattern : selected_base_patterns) {
    ...
    best_mutant->calculate_log_pvalue(ltot, this->pattern_bg_probabilities, this->pattern_counter);
    ...
    while (found_better_mutant) {
      ...
      for (int p = 0; p < pattern_length; p++) {
        ...
        for (auto r : IUPACAlphabet::get_similar_iupac_nucleotides(c)) {
          ...
          mutated_pattern->calculate_log_pvalue(ltot, this->pattern_bg_probabilities,
          ...
          if (mutated_pattern->get_log_pvalue() < best_log_pvalue) {
            ...
            std::cout << "\t" << IUPACPattern::toString(best_mutant->get_pattern(), pattern_length)
                      << "\t" << best_mutant->get_log_pvalue() << std::endl;
          } else {
...
//    }
    ...
    if (best.count(best_mutant->get_pattern()) == 0
        && seen.count(best_mutant->get_pattern()) == 0) {
      best_iupac_patterns.push_back(best_mutant);
      best.insert(best_mutant->get_pattern());
      seen.insert(best_mutant->get_pattern());

      std::cout << "optimization: " << BasePattern::toString(pattern) << " -> "
                << IUPACPattern::toString(best_mutant->get_pattern(), pattern_length) << std::endl;
    } else {
      std::cout << "optimization: " << BasePattern::toString(pattern) << " removed" << std::endl;
      delete best_mutant;
      best_mutant = NULL;
...
void Peng::printShortMeme(std::vector<IUPACPattern*>& best_iupac_patterns,
                          const std::string output_filename, BackgroundModel* bg_model) {
  ...
  if (myfile.is_open()) {
    ...
    for (auto pattern : best_iupac_patterns) {
      ...
      myfile << "letter-probability matrix:" << " alength= " << 4 << " w= "
             << pattern->get_pattern_length() << " nsites= " << pattern->get_sites() << " bg_prob= "
             << pattern->get_bg_p() << " log(Pval)= " << pattern->get_log_pvalue() << std::endl;
    ...
    std::cerr << "Unable to open output file (" << output_filename << ")!";
}

void Peng::printJson(std::vector<IUPACPattern*>& best_iupac_patterns,
                     const std::string output_filename, const std::string version_number,
